...
//    else if (isAnyEmpty(str, abbrevMarker)) return str
...
//    if (str.length - offset < maxWidth - abbrevMarkerLength) offset = str.length - (maxWidth - abbrevMarkerLength)
//    if (offset <= abbrevMarkerLength + 1) return str.substring(0, maxWidth - abbrevMarkerLength) + abbrevMarker
...
//    if (offset + maxWidth - abbrevMarkerLength < str.length) return abbrevMarker + abbreviate(str.substring(offset), abbrevMarker, maxWidth - abbrevMarkerLength)
...
//    * @since 3.2
//    */
//  def appendIfMissing(str: String, suffix: CharSequence, suffixes: CharSequence*): String = appendIfMissing(str, suffix, false, suffixes)
//
//  /**
//    * Appends the suffix to the end of the string if the string does not
...
//    * @since 3.2
//    */
//  def appendIfMissingIgnoreCase(str: String, suffix: CharSequence, suffixes: CharSequence*): String = appendIfMissing(str, suffix, true, suffixes)
//
//  /**
...
//    }) = newCodePoint // copy the first codepoint
//
...
//      newCodePoints({
//        outOffset += 1; outOffset - 1
//      }) = codepoint // copy the remaining ones
//
...
//    val last = str.charAt(lastIdx)
//    if (last == CharUtils.LF && ret.charAt(lastIdx - 1) == CharUtils.CR) return ret.substring(0, lastIdx - 1)
//    ret
//  }
//
//  /**
...
//    * @since 3.4
//    */
//  def containsAny(cs: CharSequence, searchCharSequences: CharSequence*): Boolean = {
...
//    if (cs == null || validChars == null) return false
...
//    */
//  def defaultIfBlank[T <: CharSequence](str: T, defaultStr: T): T = if (isBlank(str)) defaultStr
//  else str
//
//  /**
//    * <p>Returns either the passed in CharSequence, or if the CharSequence is
...
//    * @see StringUtils#defaultString(String, String)
//    */
//  def defaultIfEmpty[T <: CharSequence](str: T, defaultStr: T): T = if (isEmpty(str)) defaultStr
//  else str
//
  /**
...
//    */
//  def indexOfAny(str: CharSequence, searchStrs: CharSequence*): Int = {
//    if (str == null || searchStrs == null) return INDEX_NOT_FOUND
//    // String's can't have a MAX_VALUEth index.
//    var ret = Integer.MAX_VALUE
//    var tmp = 0
//    for (search <- searchStrs) {
//      if (search == null) continue //todo: continue is not supported
//      tmp = CharSequenceUtils.indexOf(str, search, 0)
//      if (tmp == INDEX_NOT_FOUND) continue //todo: continue is not supported
//      if (tmp < ret) ret = tmp
//    }
...
//      val ch = cs.charAt(i)
//      for (j <- 0 until searchLen) {
//        if (searchChars(j) == ch) if (i < csLast && j < searchLast && Character.isHighSurrogate(ch)) if (searchChars(j + 1) == cs.charAt(i + 1)) continue outer //todo: continue is not supported
//        else continue outer //todo: continue is not supported
//      }
//      return i
//    }
//    INDEX_NOT_FOUND
...
//    for (i <- 0 until strLen) {
//      val ch = seq.charAt(i)
//      val chFound = CharSequenceUtils.indexOf(searchChars, ch, 0) >= 0
//      if (i + 1 < strLen && Character.isHighSurrogate(ch)) {
//        val ch2 = seq.charAt(i + 1)
//        if (chFound && CharSequenceUtils.indexOf(searchChars, ch2, 0) < 0) return i
//      }
...
//    */
//  def join(array: Array[AnyRef], separator: String, startIndex: Int, endIndex: Int): String = {
//    if (array == null) return null
...
//    if (str == null || searchStr == null) return INDEX_NOT_FOUND
//    if (startPos > str.length - searchStr.length) startPos = str.length - searchStr.length
//    if (startPos < 0) return INDEX_NOT_FOUND
//    if (searchStr.length == 0) return startPos
...
//    if (pads == padLen) padStr.concat(str)
//    else if (pads < padLen) padStr.substring(0, pads).concat(str)
//    else {
...
//
//  private def matches(first: CharSequence, second: CharSequence) = {
//    var max = null
//    var min = null
...
//    if (ArrayUtils.isNotEmpty(prefixes)) for (p <- prefixes) {
...
//    * @since 3.2
//    */
//  def prependIfMissing(str: String, prefix: CharSequence, prefixes: CharSequence*): String = prependIfMissing(str, prefix, false, prefixes)
//
//  /**
//    * Prepends the prefix to the start of the string if the string does not
...
//    * @since 3.2
//    */
//  def prependIfMissingIgnoreCase(str: String, prefix: CharSequence, prefixes: CharSequence*): String = prependIfMissing(str, prefix, true, prefixes)
//
//  /**
...
//    */
//  @deprecated def removeAll(text: String, regex: String): String = RegExUtils.removeAll(text, regex)
//
//  /**
...
//    if (isEmpty(str) || isEmpty(remove)) return str
//    if (endsWithIgnoreCase(str, remove)) return str.substring(0, str.length - remove.length)
//    str
//  }
//
//  /**
...
//    * @deprecated Moved to RegExUtils.
//    */
//  @deprecated def removeFirst(text: String, regex: String): String = replaceFirst(text, regex, EMPTY)
//
//  /**
//    * <p>
...
//    * @deprecated Moved to RegExUtils.
//    */
//  @deprecated def removePattern(source: String, regex: String): String = RegExUtils.removePattern(source, regex)
//
//  /**
...
//    *         {@code null} if null String input
//    */
//  def replace(text: String, searchString: String, replacement: String): String = replace(text, searchString, replacement, -1)
//
//  /**
...
//    *         {@code null} if null String input
//    */
//  def replace(text: String, searchString: String, replacement: String, max: Int): String = replace(text, searchString, replacement, max, false)
//
//  /**
//    * <p>Replaces a String with another String inside a larger String,
...
//    var start = 0
//    var `end` = if (ignoreCase) indexOfIgnoreCase(text, searchString, start)
//    else indexOf(text, searchString, start)
...
//      if ( {max -= 1; max} == 0) break //todo: break is not supported
//      `end` = if (ignoreCase) indexOfIgnoreCase(text, searchString, start)
//      else indexOf(text, searchString, start)
//    }
...
//    if (timeToLive < 0) {
//      val searchSet = new util.HashSet[String](util.Arrays.asList(searchList))
//      val replacementSet = new util.HashSet[String](util.Arrays.asList(replacementList))
...
//    }
//    if (isEmpty(text) || ArrayUtils.isEmpty(searchList) || ArrayUtils.isEmpty(replacementList) || (ArrayUtils.isNotEmpty(searchList) && timeToLive == -1)) return text
...
//    for (i <- 0 until searchLength) {
//      if (noMoreMatchesForReplIndex(i) || isEmpty(searchList(i)) || replacementList(i) == null) continue //todo: continue is not supported
...
//    for (i <- 0 until searchList.length) {
//      if (searchList(i) == null || replacementList(i) == null) continue //todo: continue is not supported
...
//      for (i <- 0 until searchLength) {
//        if (noMoreMatchesForReplIndex(i) || searchList(i) == null || searchList(i).isEmpty || replacementList(i) == null) continue //todo: continue is not supported
...
//    * @deprecated Moved to RegExUtils.
//    */
//  @deprecated def replaceFirst(text: String, regex: String, replacement: String): String = RegExUtils.replaceFirst(text, regex, replacement)
//
//  /**
...
//    * @since 3.5
//    */
//  def replaceIgnoreCase(text: String, searchString: String, replacement: String): String = replaceIgnoreCase(text, searchString, replacement, -1)
//
//  /**
...
//    if (pads == padLen) str.concat(padStr)
//    else if (pads < padLen) str.concat(padStr.substring(0, pads))
//    else {
//      val padding = new Array[Char](pads)
//      val padChars = padStr.toCharArray
//      for (i <- 0 until pads) {
...
//      if (`type` == currentType) continue //todo: continue is not supported
...
//    */
//  def stripAll(strs: String*): Array[String] = stripAll(strs, null)
//
//  /**
...
//    if (pos == INDEX_NOT_FOUND || pos == str.length - 1) return EMPTY
//    str.substring(pos + 1)
//  }
//
//  /**
...
//    * @since 2.0
//    */
//  def substringBetween(str: String, tag: String): String = substringBetween(str, tag, tag)
//
//  /**
...
//    if (!ObjectUtils.allNotNull(str, open, close)) return null
