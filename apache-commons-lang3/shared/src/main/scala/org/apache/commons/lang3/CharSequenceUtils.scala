...
//    if (searchChar < Character.MIN_SUPPLEMENTARY_CODE_POINT) for (i <- start to 0 by -1) {
//      if (cs.charAt(i) == searchChar) return i
//    }
...
//      if (start == sz - 1) return NOT_FOUND
...
//    if (start < 0 || len2 < 0 || len2 > len1) return -1
//    if (len2 == 0) return start
//    if (len2 <= TO_STRING_LIMIT) if (cs.isInstanceOf[String]) return cs.asInstanceOf[String].lastIndexOf(searchChar.toString, start)
//    else if (cs.isInstanceOf[StringBuilder]) return cs.asInstanceOf[StringBuilder].lastIndexOf(searchChar.toString, start)
//    else if (cs.isInstanceOf[StringBuffer]) return cs.asInstanceOf[StringBuffer].lastIndexOf(searchChar.toString, start)
//    if (start + len2 > len1) start = len1 - len2
