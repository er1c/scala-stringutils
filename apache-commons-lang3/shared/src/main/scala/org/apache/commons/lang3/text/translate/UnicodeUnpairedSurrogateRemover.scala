...
/**
  ...
  */
@deprecated class UnicodeUnpairedSurrogateRemover extends CodePointTranslator {
  /**
  ...
  override def translate(codepoint: Int, out: Writer): Boolean = {
    if (codepoint >= Character.MIN_SURROGATE && codepoint <= Character.MAX_SURROGATE) { // It's a surrogate. Write nothing and say we've translated.
      return true
    }
